...
class UcenterMemberModel extends Model
{
    ...
    public function login($username, $password, $type = 1)
    {
        ...
        if (UC_SYNC && $username != get_username(1) && $type == 1) { //非管理员使用用户名登录，实际上是在ucenter和uctoo间同步帐号数据，不是登录
            return $this->ucLogin($username, $password);
        }
        if (UC_SYNC && $username != get_username(1) && $type == 2) { //非管理员使用邮箱登录，实际上是在ucenter和uctoo间同步帐号数据，不是登录
            return $this->ucmailLogin($username, $password);
        }
        $map = array();
        switch ($type) {
            case 1:
    ...
    public function ucLogin($username, $password)
    {
       // include_once './api/uc_client/client.php';
        ...
        $uc_user = $Ucenter->uc_user_login($username, $password);  //第三个参数加不加无所谓方法中自动判断
        //关联表内数据
        $uc_user_ref = tox_get_ucenter_user_ref('', $uc_user['0'], '');  //根据ucenter表中的uid找关联表中有没有数据
        //检测关联表数据完整度
        if ($uc_user_ref['uid'] && $uc_user_ref['uc_uid'] && $uc_user[0] > 0) {
            return $uc_user_ref['uid'];   //关联表有完整关联的uctoo数据就返回本地uid不用后续注册过程
        }
        //本地帐号信息
    ...
    public function ucmailLogin($username, $password)
    {
        ...
        if ($uc_user[0] < 0 && $tox_user['id'] > 0) {
            //写入UC
            if(empty($tox_user['username'])){  //如果uctoo帐号是用email注册的初始用户名会为空，注册ucenter时默认用邮箱@之前的部分做用户名
    ...
    public function getLocal_byEmail($username, $password)
    {
        $map = array();
        $map['email'] = $username;
