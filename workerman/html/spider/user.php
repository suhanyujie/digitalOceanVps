...
{	
	echo 123;
	var_dump(cache::get_instance()).'2-';
	
    // 如果队列为空, 从数据库取一些
    if (!cache::get_instance()->lsize($key)) 
    {
        $sql = "Select `username`, `{$key}_uptime` From `user` Order By `{$key}_uptime` Asc Limit {$count}";
        $sql = "Select `username`, `{$key}_uptime` From `user` where headimg is null Order By `{$key}_uptime` Asc Limit {$count}";
        $rows = db::get_all($sql);
        foreach ($rows as $row) 
        {
...
function save_user_index($worker = null)
{
    ...
    //$username = 'suhanyujie';   // su-yi-yu-82
